...
def main():
    ...
    parser.add_argument("--config", required=True, help="Path to YAML config, e.g. configs/exp1_pca_rf.yaml")
    ...
    # Build PCA + RF pipeline
    p = cfg["pca"]
    m = cfg["model"]

    pipeline = Pipeline(
        steps=[
            ("pca", PCA(n_components=int(p["n_components"]), whiten=bool(p["whiten"]), random_state=rs)),
    ...
    print(f"Training PCA({p['n_components']}) + RandomForest...")
    ...
    def eval_split(Xs, ys):
        ...
        proba = pipeline.predict_proba(Xs)
    ...
    val_acc, val_pred, val_conf, val_cm, val_report = eval_split(X_val, y_val)
    test_acc, test_pred, test_conf, test_cm, test_report = eval_split(X_test, y_test)
    ...
    metrics = {
        ...
        "pca_n_components": int(p["n_components"]),
        "pca_whiten": bool(p["whiten"]),
    }
    ...
    print(f"PCA components: {p['n_components']}, whiten: {p['whiten']}")
